    ...
    public class DbBase : System.IDisposable
    {
        ...
        public DataSet ExecuteDatasetNTx(object connect, MethodBase methodBase, ParamData paramData)
        {
            ...
            try
            {
                ...
                {
                    using (ZumNet.Framework.Data.OracleHelper dbHelper = new ZumNet.Framework.Data.OracleHelper())
                    {
                        ...
                        {
                            ds = dbHelper.ExecuteDataset((OracleConnection)connect, paramData.CommandType, paramData.QueryString, paramData.CommandTimeout, paramData.SrcTable, paramData.SqlParams);
                        }
                    }
        ...
        /// <param name="paramData"></param>
        /// <returns></returns>
        public OracleDataReader ExecuteReaderNTx(object connect, MethodBase methodBase, ParamData paramData)
        {
            TransactionOptions options = new TransactionOptions();
            options.Timeout = TimeSpan.FromSeconds(paramData.CommandTimeout);
            options.IsolationLevel = System.Transactions.IsolationLevel.ReadUncommitted; //격리수준
            ...
            try
            {
                ...
                using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Suppress, options))
                {
                    using (ZumNet.Framework.Data.OracleHelper dbHelper = new ZumNet.Framework.Data.OracleHelper())
                    {
                        ...
                        {
                            dr = dbHelper.ExecuteReader((OracleConnection)connect, paramData.CommandType, paramData.QueryString, paramData.CommandTimeout, paramData.SqlParams);
                        }
                    }
                    scope.Complete(); //형식을 맞추기 위해 Suppress 경우도 선언
                }
        ...
        /// <param name="paramData"></param>
        /// <returns></returns>
        public OracleDataReader ExecuteReaderTx(object connect, MethodBase methodBase, ParamData paramData)
        {
            TransactionOptions options = new TransactionOptions();
            options.Timeout = TimeSpan.FromSeconds(paramData.CommandTimeout);
            options.IsolationLevel = System.Transactions.IsolationLevel.ReadCommitted; //격리수준
            ...
            try
            {
                ...
                using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Suppress, options))
                {
                    using (ZumNet.Framework.Data.OracleHelper dbHelper = new ZumNet.Framework.Data.OracleHelper())
                    {
                        ...
                        {
                            rt = Convert.ToString(dbHelper.ExecuteScalar((OracleConnection)connect, paramData.CommandType, paramData.QueryString, paramData.CommandTimeout, paramData.SqlParams));
                        }
                    }
                    scope.Complete(); //형식을 맞추기 위해 Suppress 경우도 선언
                }
            ...
            try
            {
                ...
                using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Suppress, options))
                {
                    using (ZumNet.Framework.Data.OracleHelper dbHelper = new ZumNet.Framework.Data.OracleHelper())
                    {
                        ...
                        {
                            int i = dbHelper.ExecuteNonQuery((OracleConnection)connect, paramData.CommandType, paramData.QueryString, paramData.CommandTimeout, paramData.SqlParams);
                        }
                    }
                    ...
                    int iCnt = 0;
                    char cDiv = (char)1;
                    foreach (OracleParameter p in paramData.SqlParams)
                    {
